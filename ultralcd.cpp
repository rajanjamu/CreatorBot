...
static void Calibrate_Bed_Manually()
{   refresh_cmd_timeout();

    if (A == '0')
    {  enquecommands_P(PSTR("G28"));          
       lcd.setCursor(0,1);
       lcd_printPGM(PSTR("Rotate the encoder ")); 
       lcd.setCursor(0,2);
       lcd_printPGM(PSTR("to move hot-end."));
       A= '1';
    }
    if (encoderPosition != 0)
    {
        move_bed_point += (int)encoderPosition;
        if (move_bed_point < 0)
        move_bed_point = 12;
        if (move_bed_point > 12)
        move_bed_point = 0;  
    ...
    while(current_point != move_bed_point)
    {
      switch(move_bed_point)
      {                 
        case 0 :enquecommands_P(PSTR("G1 Z5 F200 \n G1 X0 Y0 F2000 \n G1 Z0 F200"));
                lcd.setCursor(0,1);
	        lcd_printPGM(PSTR("1)Front Left Corner   "));
        break;
        case 4 :enquecommands_P(PSTR("G1 Z5 F200 \n G1 X150 Y0 F2000 \n G1 Z0 F200"));
                lcd.setCursor(0,1);
	        lcd_printPGM(PSTR("2)Front Right Corner "));
        break;
        case 8 :enquecommands_P(PSTR("G1 Z5 F200\n G1 X150 Y150 F2000 \n G1 Z0 F200"));
                lcd.setCursor(0,1);
	        lcd_printPGM(PSTR("3)Back Right Corner   "));
        break;
        case 12:enquecommands_P(PSTR("G1 Z5 F200\n G1 X0 Y150 F2000 \n G1 Z0 F200"));
                lcd.setCursor(0,1);
                lcd_printPGM(PSTR("4)Back Left Corner   "));
        break;
        default :
        break;  
      }
...
static void Feed_Filament()
{ refresh_cmd_timeout();

if(degHotend(active_extruder)<extrude_mintemp)
 {
      lcd.setCursor(0,1);
      lcd_printPGM(PSTR("Preheat PLA and try   "));  
 ...
 {  
    if (feed_on == '0')
    {  enquecommands_P(PSTR("G1 Z5 F200"));          
       feed_on= '1';
    }
    
    if (encoderPosition != 0)
    {
        Move_Filament += (int)encoderPosition;
         if (Move_Filament < 0)
        Move_Filament = 0 ;
        if (Move_Filament > 1)
        Move_Filament = 0;
    ...
    {              
        ...
        lcd.setCursor(0,1);
        lcd_printPGM(PSTR("Press Encoder to stop  ")); 
        lcd.setCursor(0,2);
        lcd_printPGM(PSTR("                   "));
...
static void lcd_prepare_menu()
{
    ...
    MENU_ITEM(submenu, "Calibrate bed",  Calibrate_Bed_Manually);
    MENU_ITEM(submenu, "Feed Filament",  Feed_Filament);
